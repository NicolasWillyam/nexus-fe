...
Chức năng này hiển thị một thẻ điểm tổng hợp (Score Card) cho một mã cổ phiếu, giúp người dùng đánh giá nhanh cổ phiếu đó mà không cần đọc báo cáo tài chính chi tiết. Thẻ gồm một Điểm Tổng (thang 0–100) và 3 điểm thành phần: Hiệu Suất (lợi nhuận 1 năm), Động Lượng (đà tăng giá), Rủi Ro (mức độ biến động/an toàn).
...
Chức năng này giúp người dùng ra quyết định đầu tư nhanh hơn, tổng hợp nhiều chỉ số phức tạp thành một con số dễ hiểu, hỗ trợ so sánh nhanh giữa các mã.
...
Hiển thị Điểm Tổng và 3 điểm thành phần dưới dạng Score Card.
...
risk_metrics — biến động, sụt giảm tối đa, Sharpe ratio, beta
 Lưu ý quan trọng: API không trả sẵn 4 điểm Quality/Momentum/Value/Risk như ví dụ minh họa trong đề bài, và cũng không có dữ liệu định giá (P/E, P/B...) nên không thể tính điểm "Value" có căn cứ. Vì vậy tôi đã tự tính 3 điểm thành phần (Hiệu Suất, Động Lượng, Rủi Ro) từ dữ liệu thô — công thức ở mục 7.
...
Tự thiết kế công thức quy đổi dữ liệu thô sang thang điểm 0-100:
Động Lượng = rsi_14 (đã ở thang 0-100), +10 nếu MACD đang bullish, -10 nếu không, giới hạn lại trong khoảng 0-100.
Rủi Ro (điểm càng cao = càng an toàn) = 100 - annual_volatility*100 - |max_drawdown|*100.
Hiệu Suất = 50 + total_return_1y*100 (mốc 50 = hoà vốn).
Bỏ chỉ số "Value" vì không có dữ liệu định giá.
...
Hiển thị Điểm Tổng to ở giữa, 3 điểm thành phần dạng thanh tiến trình.
Dịch toàn bộ nhãn hiển thị sang tiếng Việt cho khớp với cả dashboard.
...
1
Nhập AAPL
Hiển thị Score Card với điểm số
Hiện đúng: 43/100, Hiệu Suất 81, Động Lượng 16, Rủi Ro 61
PASS
2
Nhập MSFT
Hiển thị Score Card với điểm số
Hiện đúng: 57/100, Hiệu Suất 45, Động Lượng 95, Rủi Ro 33
PASS
3
Nhập mã không tồn tại (ABCD)
Hiển thị thông báo lỗi/không tìm thấy, không crash
API trả lỗi cho mã không hợp lệ, code chạy vào nhánh catch, hiện "Không thể tải dữ liệu điểm số. Vui lòng thử lại." — không crash
PASS
4
Tắt backend (dừng uvicorn)
Hiển thị thông báo lỗi, không crash
Hiện đúng thông báo lỗi, không crash trang
PASS
...
API không trả sẵn 4 điểm Quality/Momentum/Value/Risk như đề bài minh họa, phải tự thiết kế công thức quy đổi từ dữ liệu thô và tự quyết định bỏ chỉ số "Value" vì không có dữ liệu định giá.
...
Commit: feat: add stock score card
